...
(SELECT 
    ...
    FROM archimedes.habs GROUP BY robot) AS robot_habs
NATURAL JOIN
  (SELECT robot,    
...
(SELECT robot AS number, COUNT(CASE 
    ...
    END) AS sub_count
    FROM (archimedes.schedule NATURAL JOIN (SELECT ARRAY[r1, r2, r3, b1, b2, b3] AS robots FROM archimedes.schedule WHERE MATCH = $1) AS robot_list), archimedes.teams
